...
ger_word_freq  = Counter(german_word_corpus)
non_singleton_german_corpus = [word for word in ger_word_freq if ger_word_freq[word] > 1]
non_singleton_german_corpus.append("<UNK>")

eng_word_freq  = Counter(english_word_corpus)
non_singleton_eng_corpus = [word for word in eng_word_freq if eng_word_freq[word] > 1]
non_singleton_eng_corpus.append("<UNK>")

german_word_vocab = Vocab.from_corpus([non_singleton_german_corpus])
english_word_vocab = Vocab.from_corpus([non_singleton_eng_corpus])
...
for (german_list, english_list) in training_sentences:
	...
	indexed_train.append((indexed_german_list, indexed_eng_list))
...
# 	perplexity = math.exp((loss_value * batch_size)/float(validation_num_op_tokens))
# 	return perplexity
	...
	for i in xrange(num_epochs):
		...
		for batch in training_batches:
			...
			if num_batches % 100 == 0:
				...
				f = open("translations_batch_unk_drop_512.txt", "a")
				f.write(to_write + "\n")
				f.close()
                        if num_batches % 1000 == 0:
				model_filename = "model_batch_unk_drop_512/model_epoch_" + str(i) + "_sample_" + str(num_samples)
